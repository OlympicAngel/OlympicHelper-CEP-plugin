...
async function SilenceCut() {
    ...
    for (let index = 0; index < cut_arr.length; index++) {
        ...
        timeSum += duration;
        removeSum += a_cut.end_at - a_cut.start_at;
    ...
    console.log(`%c Done: ${cut_arr.length} cuts removed ${removeSum / 60}m from clip / avg cut:${timeSum / cut_arr.length}`, "border-left:solid 1vmin lime; color: lime")
