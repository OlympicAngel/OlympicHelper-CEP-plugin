...
async function SilenceCut() {
    ...
    let responseTimeout = setTimeout(ffmpeg_timeout, 5 * 1000)
    let gotFullResponse = false;
    while (!gotFullResponse) {
        clearTimeout(responseTimeout);
        responseTimeout = setTimeout(ffmpeg_timeout, 5 * 1000)
    ...
    let removeSum = Math.floor(cut_arr.reduce((sum, current) => { return sum + current.end_at - current.start_at }, 0) * 100) / 100;
    await new Promise(resolve => evalScript(`$._PPP_.AutoCut_end();`, resolve));
    console.log(`%c Done: ${cut_arr.length} cuts removed ${removeSum / 60}m from clip / avg cut:${silenceCut_timing.timePerCut_sum / cut_arr.length}`, "border-left:solid 1vmin lime; color: lime")
