...
async function SilenceCut() {
    ...
    await new Promise(resolve => evalScript(`$._PPP_.SilenceCut_start();`, resolve));
    ...
    $("div.cutLoader p").html(`<label class="EN">Detected </label><label class="HE">זוהו </label>
        ...
        <label class="EN"> cuts</label><label class="HE"> קאטים</label>...<br>
            <small><label class="EN">Please wait..</label><label class="HE">נא המתן..</label></small>`);
    ...
    await sleep(100 + detectStr.length * 3);
    ...
    $("div.cutLoader").css("color", "red")
    $("div.cutLoader p").html(`<label class="EN">Generates timeline</label><label class="HE">מייצר ציר זמן </label>
            <small>
            <label class="EN">Premiere might freeze for a sec, don't worry..</label>
            <label class="HE">פרימייר עשוייה לקפוא לשנייה, אין מה לדאוג..</label>
            </small>`);
    ...
    const raw_config = await new Promise(resolve => evalScript(`$._PPP_.SilenceCut_getMeta()`, resolve));
    if (raw_config.startsWith("Error:")) {
        jsx_error(filePath);
    ...
    /** @type {{height: Number mediaPath: String numAudioTracks: Number sampleRate: Number timebase: Number totalDuration: Number width: Number inPoint: Number}} */
    let config;
    try {
        config = JSON.parse(raw_config);
    } catch (e) {
        console.error("Failed to parse metadata", raw_config)
    }
    const name = config.mediaPath.replaceAll("/", "\\").split("\\").pop();
    const videoFile = {
        name,
        path: config.mediaPath
    };

    const xmlString = generateNonSilentXML(videoFile, [{ start_at: 0, end_at: config.inPoint }, ...cut_arr], config);

    //save to file
    const projPath = (await evalScript("app.project.path")).replaceAll("\\", "/").split("/").reverse().splice(1).reverse().join("/")
    const SilenceCutDir = projPath + "/SilenceCut/"
    let xmlPath = SilenceCutDir + "SilenceCut_" + (Math.random() * 10).toString().replace(".", "") + ".xml";

    //create dir if not exists
    if (!fs.existsSync(SilenceCutDir))
        fs.mkdirSync(SilenceCutDir, { recursive: true });

    fs.writeFileSync(xmlPath, xmlString, "utf-8")

    const res = await new Promise(resolve => evalScript(`$._PPP_.SilenceCut("${NormalizePath(xmlPath)}")`, resolve));
    if (res.startsWith("Error:")) {
        ...
        $("div.cutLoader p").html(`
            <center>
                <label class="EN" style="color:red">FAILED, please send logs to Olympic.</label>
                <label class="HE" style="color:red">פעולה נכשלה יש לשלוח לוגים לאולימפיק.</label>
            </center>`);
    ...
    csInterface.requestOpenExtension("com.OlympicAngel.OlympicHelper.ModalDialog", "");
    ...
    const exe_duration = Math.floor((new Date() - startAt) / 1000);

    await new Promise(resolve => evalScript(`$._PPP_.SilenceCut_end();`, resolve));
    console.log(`%c Done: ${cut_arr.length} cuts removed ${removeSum}m from clip / avg cut:${exe_duration / cut_arr.length}`, "border-left:solid 1vmin lime; color: lime")
   ...
   <small style="color:gray">
       <label style="opacity:0.5;">${exe_duration / cut_arr.length} avg per cut.</label>
       <label class="EN">Returns to Main Menu in 8 secs..</label>
       <label class="HE">חוזר למסך הראשי עוד 8 שניות..</label>
   </small>`);
...
async function SilenceCut_getAudioTracks() {
    ...
    if (s_c_margin.getAttribute("ini") == "false") {
        s_c_margin.setAttribute("ini", true)
        s_c_margin.value = ~~frameTimeMS;
    }
...
async function FFmpeg() {
    ...
    if (analyzeTrackNum == "a") {
        const audioInputs = Array.from({ length: maxAudioTracks }, (_, i) => `[0:a:${i}]`).join("");
        args.filter_value = `${audioInputs}amerge=inputs=${maxAudioTracks}[aout];[aout]silencedetect=noise=${volume}dB:duration=${min_time}:mono=false`
    }
...
function HandleFFmpegData(data, detectStr, checkInterval, rawData) {
    ...
    (async () => {
        for (let index = 0; index <= 10; index++) {
            countElement.text(Math.floor((cut_count - newCuts * (1 - index / 11))).toLocaleString())
            await sleep(checkInterval / 11);
        }
    })();
