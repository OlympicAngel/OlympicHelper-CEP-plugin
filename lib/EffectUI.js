class EffectUI {
    ...
    Render() {
        ...
        if (!this.renderOverride) {
            ...
            var saveDefaultBtn = document.createElement("saveAs");
            saveDefaultBtn.innerHTML = `<label class="HE">קבע כברירת מחדל</label><label class="EN">Set as default</label>`;
            saveDefaultBtn.onclick = SavePreset.bind(null, this, "");
            saveDefaultBtn.oncontextmenu = RemovePreset.bind(null, this, "");
            mainDiv.appendChild(saveDefaultBtn);
    ...
    async GenerateFX(e) {
        $("div.cutLoader").show();
        $("div.cutLoader p").html(`<label class="EN">Loading FX..</label><label class="HE">טוען אפקט..</label>
        <span id="cutCount">${this.name[~~HE]}</span><br>
        <center style="color:red"><span id="clipCount">0/--</span> <span id="progress">0%</span></center>
            <small><label class="EN">Please wait and don't do ANY action!</label><label class="HE">נא המתן ואל
                תבצע אף
                פעולה!</label></small>`);

        EffectUI.loader = { clip: $("#clipCount"), progress: $("#progress") };

        e.target.blur();
        let frameTime = await new Promise(resolve => evalScript('$._PPP_.GetClipFrameTime()', resolve));
        ...
        const output = await new Promise(resolve => evalScript('$._PPP_.' + this.pppFuncName + '(' + args + ')', resolve));
        jsx_error(output);
        console.log(output)
        $("div.cutLoader").hide();
    }
...
EffectUI.loader = { clip: null, progress: null };
csInterface.addEventListener("fx_clip_done", (e) => { EffectUI.loader.clip.text(e.data) });
csInterface.addEventListener("fx_progress", (e) => { EffectUI.loader.progress.text(e.data + "%") });
...
var silenceCut = new EffectUI(["Silence Cut", "חיתוך שקט"], "autoCut");
silenceCut.renderOverride = (function (div) {
    var container = document.getElementById("autoCut");
    var clone = container.cloneNode(true);
    div.appendChild(clone);
    container.remove();
    const presets = div.querySelectorAll("[preset]")
    presets.forEach(p => {
        let memo = localStorage["silenceCut" + p.getAttribute("preset")]
        if (!memo)
            return;

        p.innerText = JSON.parse(memo).name;
    })
});
all_fvx.push(silenceCut);
...
camMove.AddUI(["Max Rotation", "סיבוב מקסימלי"], "mRotate", 5, "°", 0, 30, 1, true);
...
var pitchRandom = new EffectUI(["Random Pitch", "פיצ' אקראי"], "rndPitch", "RandomPitcher");
pitchRandom.AddUI(["Base Pitch", "פיצ' בסיסי"], "base", 0, "%", -50, 100, 1, false);
pitchRandom.AddUI(["Random Range", "טווח אקראיות"], "rnd", 35, "%", 0, 70, 1, false);
all_fvx.push(pitchRandom);
